			...
			<div class="col-xs-3">
				<div class="panel panel-default">
					...
					<div class="panel-body">
								...
								<c:forEach items="${instructsList }" var="list">
									<tr>
										<td><c:if test="${list.proType=='1' }">单选题</c:if> <c:if
												test="${list.proType=='2' }">多选题</c:if> <c:if
												test="${list.proType=='3' }">填空题</c:if> <c:if
												test="${list.proType=='4' }">简答题</c:if></td>
										<td>${list.instruct }</td>
										<td>${list.number }</td>
										<td>${list.allScore }</td>
									</tr>
						...
						<div class="row">
							<a id="showModal" class="btn btn-warning" data-toggle="modal"
								data-target="#myModal"> 生成试卷</a>
						</div>
					</div>
						...
						<div class="row">
							<div class="col-xs-8">
								<div class="page-header">
									<h1>
										<a
											href="${pageContext.request.contextPath}/paper/paper/item/1.do">
											<< 返回试题列表</a> <small>继续添加试题</small>
									</h1>
								</div>
							</div>
							<div class="col-xs-4">
								<ul class="list-group">
									<li class="list-group-item">单选题：已选<span
										class="label label-danger num">${paperCart.singleNumber}个</span></li>
									<li class="list-group-item">多选题：已选<span
										class="label label-danger num">${paperCart.multChoiceNumber}个</span></li>
									<li class="list-group-item">判断题：已选<span
										class="label label-danger num">${paperCart.judgeNumber}个</span></li>
									<li class="list-group-item">简答题：已选<span
										class="label label-danger num">${paperCart.questionNumber}个</span></li>
								</ul>
							</div>
						</div>
												...
												<div class="panel panel-default item">
													<div class="panel-heading">
														<div class="row">
															...
															<div class="col-xs-2">
																<button class="btn btn-primary remove" id="${pro.id}">删除</button>
															</div>
														</div>
													</div>
													<div class="panel-body">
														...
														<hr>
														A:${pro.optA }<br> B:${pro.optB }<br>
														C:${pro.optC }<br> D:${pro.optD }<br>
													</div>
												</div>
											</div>
										</c:forEach>
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<form class="form-horizontal"
						action="${pageContext.request.contextPath}/paper/paper/save.do"
								...
								<ul>
									<c:forEach items="${instructsList }" var="list">
										<li class="list-group-item"><c:if
												test="${list.proType=='1' }">单选题：</c:if> <c:if
												test="${list.proType=='2' }">多选题：</c:if> <c:if
												test="${list.proType=='3' }">填空题：</c:if> <c:if
												test="${list.proType=='4' }">简答题：</c:if> 预计 <span
											class="label label-danger num"> ${list.number } 个</span></li>
									</c:forEach>
								</ul>
							</div>
							<div class="col-xs-6">
								<ul class="list-group">
									<li class="list-group-item">单选题：现有<span
										class="label label-danger num">${paperCart.singleNumber}个</span></li>
									<li class="list-group-item">多选题：现有<span
										class="label label-danger num">${paperCart.multChoiceNumber}个</span></li>
									<li class="list-group-item">判断题：现有<span
										class="label label-danger num">${paperCart.judgeNumber}个</span></li>
									<li class="list-group-item">简答题：现有<span
										class="label label-danger num">${paperCart.questionNumber}个</span></li>
								</ul>
							</div>
				...
				<div class="modal-footer">
					<input type="submit" class="btn btn-warning" value="提交试卷">
