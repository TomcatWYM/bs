			...
			<div class="col-sm-3">
				<div class="panel panel-default">
					...
					<div class="panel-body">
						...
						<div class="row">
							<a
								href='${pageContext.request.contextPath}/paperCart/paperCart.do'
								target="_blank" class="btn btn-primary">查看试卷栏</a>
							<a
								href='${pageContext.request.contextPath}/problem/problem.do'
								target="_blank" class="btn btn-warning">创建试题</a>
						</div>
					</div>
				</div>
				 <!--搜索提示栏 -->
						 ...
						 <div class="panel-body">
    						 <div class="form-group">
    						 	<div class=row>
    						 		...
    						 		<div class="col-xs-2">
    						 			 <button  id="problemList" class="btn btn-default">搜索</button>
    						 		</div>
    						 	</div>
						      </div>
						</div>
								...
								<div class="panel panel-default item">
									<div class="panel-heading">
										<div class="row">
											...
											<div class="col-sm-3 create">
												入库日期:
												<fm:formatDate pattern="yyyy-MM-dd"
													value="${pro.createDate}" />
											</div>
											<div class="col-sm-2 type">试卷类型:${pro.type }</div>
											<div class="col-sm-3">
												<button class="btn btn-primary add" id="${pro.id}">添加到试卷篮</button>
											</div>
										</div>
									...
									<div class="panel-body">
								  	...
								  	题目:<span id="${pro.id}title" style="color: blue;">${pro.title}</span>
											<hr>
								    A:<span id="${pro.id}A">${pro.optA }</span>
											<br>
								    B:<span id="${pro.id}B">${pro.optB }</span>
											<br>
								    C:<span id="${pro.id}C">${pro.optC }</span>
											<br>
								    D:<span id="${pro.id}D">${pro.optD }</span>
											<hr>
										</c:if>
								  	...
								  	题目:<span id="${pro.id}title" style="color: blue;">${pro.title}</span>
										...
										<button class="btn btn-sm btn-info"
											onClick="$(this).siblings('div').toggle();">答案</button>

										<span style="float: right"> <!-- 删除试卷
					...
					<div class="modal-body">
						<form class="form-horizontal" role="form" id="form2" method="post">
							<div class="form-group">
								...
								<div class="col-sm-10">
									<textarea id="title" type="text" class="form-control" rows="3"
							...
							<div class="form-group">
								...
								<div class="col-sm-10">
									<textarea id="ans" type="text" class="form-control" rows="3"
										name="answer" placeholder="参考答案"></textarea>
								</div>
							</div>
							<div class="form-group">
	...
	<script type="text/javascript">
		$(function() {
			$(".search").on("click",function(){
				...
				var id=$(this).attr("id");
				alert(url)
				$.post("bs/paperCart/paperCart/action/add.do",{"id":id});
				$(this).html("已添加");
			}); 
			$("#problemList").click(function(){
				$(".data").remove();
				var key = $("#key").val();
				$.post("${pageContext.request.contextPath}/problem/problem/search/json.do",{"key":key},function(data){
					$(data).each(function(){
						var tr = $("<tr class='data'><td>"+this.type+"</td><td>"+this.title+ "</td><td>"+this.optA+ "</td><td>"+this.optB+ "</td><td>"+this.optC+ "</td><td>"+this.optD+ "</td><td>"+this.answer+"</td><td><button class='btn btn-warning search' id='"+this.id+"'  >添加到试卷</button></td></tr>")
						$("#table1").append(tr);
					});
				});
			...
			$(".add").click(function() {
				...
				if ($(this).html() == '添加到试卷篮') {
					//想session中添加试题
					$.post("/bs/paperCart/paperCart/action/add.do", {
						"id" : id
					});
					$(this).html('已添加');
				} else {
					$.post("/bs/paperCart/paperCart/action/remove.do", {
						"id" : id
					});
					$(this).html('添加到试卷篮');
				}
		...
		$(function() {
			...
			$("#sub").click(
					function() {
						$.post("../../problem/update/json.do", $("#form2")
